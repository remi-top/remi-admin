    ...
    try {
      ...
      const res = await userStore.login(
        toRaw({
          ...
          redirectUrl:route.query.redirectUrl ? decodeURIComponent(route.query.redirectUrl) : '',
          appKey:route.query.appKey || appKey,
          mode: 'none', //不要默认的错误提示
        })
      );
      if (res) {
        notification.success({
          ...
          description: `${t('sys.login.loginSuccessDesc')}: ${res?.loginName}`,
          duration: 3,
        });
      }
    } catch (error) {
