<template>
  ...
  <UpdatePassword @register="registerPassModal" @success="initUserDetail" />
</template>
<script lang="ts" setup>
  ...
  import UpdatePassword from './commponents/UpdatePassword.vue';
  ...
  function initUserDetail() {
    ...
    getUserData().then((res) => {
      if (res) {
        ...
        if(res.phone){
          userDetail.value.phoneText = res.phone.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2');
        }
      }
    });
  }
