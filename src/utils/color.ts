...
export function lightenB(color, amount) {
  // 移除 '#' 符号，如果存在的话
  color = color.indexOf('#') >= 0? color.substring(1, color.length) : color;
  ...
  // 增加蓝色分量的亮度
  const newBlue = Math.min(255, rgb[2] + amount);

  // 将修改后的 RGB 分量转换回十六进制颜色代码
  const newColor = `#${rgb[0].toString(16).padStart(2, '0')}${rgb[1].toString(16).padStart(2, '0')}${newBlue.toString(16).padStart(2, '0')}`;
