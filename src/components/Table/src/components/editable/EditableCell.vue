<script lang="tsx">
  ...
  import { pick, set, cloneDeep } from 'lodash-es';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getWrapperStyle = computed((): CSSProperties => {
        ...
        const {isBlurSubmitEditData,isChangeSubmitEditData} = props.column
         if (isBlurSubmitEditData || isChangeSubmitEditData) {
          return {
            width: 'calc(100% - .5rem)',
          };
        }
      ...
      const isHiddenEditIcon = computed(() => {
        const { hiddenEditIcon } = props.column || {};
        return hiddenEditIcon;
      });
      ...
      function handleEdit() {
        // if (unref(getDisable))return
        if (props.column?.hiddenEditIcon) {
          const { column, index, record } = props;
          let row = cloneDeep(record)
          if (!record) return false;
          const { key, dataIndex } = column;
          if (!key && !dataIndex) return;
          const dataKey = (dataIndex || key) as string;
          beforeEditValue.value = row[dataKey]
        }
      ...
      async function handleChange(e: any) {
        ...
        if (props.column.isChangeSubmitEditData) {
          handleSubmit()
          return
        }
        handleSubmitRule();
      ...
      async function handleSubmit(needEmit = true, valid = true) {
        ...
        if (valid) {
          ...
          if (!isPass) {
            if (props.column?.hiddenEditIcon) { 
              set(record, dataKey, beforeEditValue.value);
              needEmit && table.emit?.('edit-end', { record, index, key: dataKey, value:beforeEditValue.value });
              isEdit.value = false;
            }
            return false
          };
        ...
        // console.log('result',dataKey,value);
        set(record, dataKey, value);
      ...
      function onClickOutside() {
        // change 选择会触发 onClickOutside
        if (props.column?.isChangeSubmitEditData)return
      ...
      return (
        <div class={this.prefixCls}>
          ...
          >
            ...
            {!this.column.editRow && !this.isHiddenEditIcon && <FormOutlined class={`${this.prefixCls}__normal-icon`} />}
          </div>
          {this.isEdit && (
            <Spin spinning={this.spinning}>
              <div class={`${this.prefixCls}__wrapper`} v-click-outside={this.onClickOutside}>
                ...
                />
                {!this.getRowEditable && !this.isHiddenEditIcon && (
